...
public class PathCopyingPersistentAvlTreeMapTest {
  private static final TestStringSortedMapGenerator mapGenerator =
      new TestStringSortedMapGenerator() {
        ...
        protected SortedMap<String, String> create(Map.Entry<String, String>[] pEntries) {
          @Var PersistentSortedMap<String, String> result = PathCopyingPersistentAvlTreeMap.of();
  ...
  public static junit.framework.Test suite() {
    ...
    suite.addTest(new JUnit4TestAdapter(PathCopyingPersistentAvlTreeMapTest.class));

    suite.addTest(
        NavigableMapTestSuiteBuilder.using(mapGenerator)
            .named("PathCopyingPersistentAvlTreeMap")
            .withFeatures(
  ...
  private void remove(@Nullable String context, String key) {
    PersistentSortedMap<String, String> oldMap = map;
    int oldMapSize = oldMap.size();
    String oldMapStr = oldMap.toString();
  ...
  public void testRandom() {
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      String context = "initial inserts, i=" + i + ", op=put, key=" + key + ", value=" + value;
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      if (rnd.nextBoolean()) {
        ...
        String context = "random ops, i=" + i + ", op=put, key=" + key + ", value=" + value;

        put(context, key, value);
        comparison.put(key, value);
      } else {

        String context = "random ops, i=" + i + ", op=remove, key=" + key;
    ...
    while (!map.isEmpty()) {
      ...
      String context = "clear map, removeFirst=" + removeFirst + ", key=" + key + ", currentSize="
          + map.size();
  ...
  public void testEntrySetContains() {
    PersistentMap<String, Integer> first =
        PathCopyingPersistentAvlTreeMap.<String, Integer>of().putAndCopy("c", 3).putAndCopy("d", 4);
    PersistentMap<String, Integer> second =
        PathCopyingPersistentAvlTreeMap.<String, Integer>of().putAndCopy("b", 2).putAndCopy("c", 3);
