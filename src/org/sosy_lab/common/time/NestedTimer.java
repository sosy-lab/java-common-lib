 ...
 *
 * If you have a method A that calls a method B, this class can be used to
 * measure (1) the time spent by A and B together, (2) the time spent by B,
 * and (3) the time spent only in A (without the call to B).
 * The advantage over using two separate timers is that this allows to compute
 * average and maximum of measurements for (3). A simple use of two timers
 * would allow to compute these values only for (1) and (2); for (3) one could
 * only compute the sum time.
 *
 * This class uses the name "total" to refer to measurement (1),
 * the name "inner" to refer to measurement (2),
 * and the name "outer" to refer to measurement (3).
 * So in general "total" represents the sum of "inner" and "outer".
 *
 * The "inner" timer can never be running as long as the "total" timer is not running.
 * During a single "total" interval, the "inner" timer can be started and stopped
 * several times. This will be counted only as one "inner" interval
 * (e.g., when the average "inner" time is computed).
 * Similarly, when the "inner" timer is started and stopped several times
 * in a single "total" interval, this counts as only one "outer" interval.
 * The number of all starts and stops of the "inner" timer summed over all "outer" intervals
 * is never used by this class and is not available to the user.
 *
...
public final class NestedTimer {
   ...
   * Contrary to the other methods of this class, this method is thread-safe.
  ...
  /**
   * Check if the outer timer is running,
   * i.e., the timer is running but the inner timer is not running.
   * Contrary to the other methods of this class, this method is thread-safe.
   * This means it can be safely run from another thread.
   */
   ...
   * This reference can be used to start and stop the inner interval
   * (even multiple times).
   * The returned reference becomes invalid after a call to {@link #stopOuter()}
   * or {@link #stopBoth()} and may not be used anymore afterwards.
   */
   ...
   * Note that this is not the same as the maximum of the intervals of all inner timers.
   * If the inner timer is running, the current interval is also counted (up to the current time).
   * If the timer was never started, this method returns 0.
   */
   ...
   * Note that this is the same as the number of outer intervals.
   ...
   * If the outer timer is running, this is the time from the start of the current interval
   * up to now.
   * If the timer was never started, this method returns 0.
   */
   ...
   * Note that this is not the average of all intervals of the inner timers,
   * but basically the same as <code>getInnerSumTime() / getNumberOfIntervals()</code>.
   * If the inner timer is running, the current interval is also counted (up to the current time).
   * If the timer was never started, this method returns 0.
   */
